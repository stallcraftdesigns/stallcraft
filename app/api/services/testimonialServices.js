...
class TestimonialService {
    ...
    static async getTestimonialById(testimonialId) {
        try {
            // Check if testimonial exists in cache
            const cachedTestimonial = this.testimonials.find((t) => t.id === testimonialId);
    ...
    static async updateTestimonial(testimonialId, updatedData) {
        try {
            const timestamp = admin.firestore.FieldValue.serverTimestamp();
            const testimonialRef = db.collection("testimonials").doc(testimonialId);
